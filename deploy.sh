#!/usr/bin/env bash
# SafeChat one-command deploy → Contabo (safechat.live)
# Usage: ./deploy.sh
# Reads Supabase keys from frontend/.env.local (never committed to git).
set -euo pipefail

SERVER="root@161.97.97.34"
REMOTE_DIR="/root/safechat"
SITE_URL="https://safechat.live"
...
ssh "$SERVER" "
  ...
  docker build -q \
    ...
    -t safechat .
  docker rm -f safechat >/dev/null 2>&1 || true
  docker run -d --restart unless-stopped --name safechat --network $NETWORK safechat >/dev/null
  rm -f ./.env.build
...
if [ "$code" = "200" ]; then
  ...
  echo "⚠️  $SITE_URL returned HTTP $code — check: ssh $SERVER 'docker logs safechat --tail 50'"
  exit 1
fi
